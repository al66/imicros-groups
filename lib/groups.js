    ...
    actions: {
        ...
        addGrant: {
            ...
            handler(ctx) {
                ...
                let statement = "MATCH (f:Group { uid: {forGroupId} }) ";
                statement += "MATCH (g:Group { uid: {byGroupId} }) ";
                statement += "WHERE (g)<-[:MEMBER_OF { role: {adminRole} }]-(:User  { uid: {adminId} }) ";
                statement += "MERGE (g)-[r:GRANT]->(f) ";
                statement += "ON CREATE SET r.ruleset = {ruleset}";
                statement += "ON MATCH SET r.ruleset = {ruleset}";
                statement += "RETURN g.uid AS byGroupId, f.uid AS forGroupId, r.ruleset AS ruleset";
                statement += ";";
                return this.run(statement, params);
        ...
        removeGrant: {
            ...
            handler(ctx) {
                ...
                let statement = "MATCH (g:Group { uid: {byGroupId} })-[r:GRANT]->(f:Group { uid: {forGroupId} })";
                statement += "WHERE (g)<-[:MEMBER_OF {role: {adminRole} }]-(:User {uid: {adminId} }) ";
                statement += "DELETE r ";
                statement += "RETURN f.uid AS removed";
                statement += ";";
                return this.run(statement, params);
        ...
        access: {
            handler(ctx) {
                ...
                let statement = "MATCH (:User {uid: {userId} })-[r:MEMBER_OF]->(g:Group)";
                statement += "RETURN g.uid AS id, TYPE(r) AS relation, '' AS ruleset ";
                statement += "UNION ";
                statement += "MATCH (g:Group)-[r:GRANT]->(m:Group) ";
                statement += "WHERE (m)<-[:MEMBER_OF]-(:User {uid: {userId} }) ";
                statement += "RETURN g.uid AS id, TYPE(r) AS relation, r.ruleset AS ruleset ";
                statement += ";";
                return this.run(statement, params);
            }
        }
